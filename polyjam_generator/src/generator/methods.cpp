...
#ifndef POLYJAM_ENABLE_PARALLEL_PRUNING
#define POLYJAM_ENABLE_PARALLEL_PRUNING 1
#endif
...
std::list<int>
polyjamSequentialEquationRemoval(
    polyjam::generator::CMatrix & big_matrix,
    const polyjam::generator::CMatrix::polynomials_t & goodPolynomials,
    size_t equationCount )
{
  std::list<int> usedEquations;
  for( size_t i = 0; i < equationCount; i++ )
    usedEquations.push_back(static_cast<int>(i));
  ...
  while(removedSome)
  {
    ...
    while(ueIt != usedEquations.end())
    {
      ...
      polyjam::generator::CMatrix subMatrix = big_matrix.subMatrix(usedEquations);
      subMatrix.reduce();
      
      if( !subMatrix.contains(goodPolynomials) )
      {
        for( int i = static_cast<int>(removed.size())-1; i >= 0; i-- )
      ...
      {
        toRemove *= 2;
        while( toRemove > static_cast<int>(usedEquations.size()) )
          toRemove /= 2;
      }
    }
...
bool
polyjamVerifyPrunedRows(
    polyjam::generator::CMatrix & big_matrix,
    const std::list<int> & usedEquations,
    const polyjam::generator::CMatrix::polynomials_t & goodPolynomials )
{
  polyjam::generator::CMatrix subMatrix = big_matrix.subMatrix(usedEquations);
  subMatrix.reduce();
  return subMatrix.contains(goodPolynomials);
}

std::list<int>
polyjamPruneEquations(
    polyjam::generator::CMatrix & big_matrix,
    const polyjam::generator::CMatrix::polynomials_t & goodPolynomials,
    size_t equationCount )
{
#if POLYJAM_ENABLE_PARALLEL_PRUNING
  std::cout << "Trying parallel row-basis pruning of unnecessary equations." << std::endl;
  std::vector<int> allRows = polyjam::generator::pruning::allRows(equationCount);
  std::vector<int> basisRows = polyjam::generator::pruning::rowBasisParallel(
      big_matrix,
      allRows,
      POLYJAM_PARALLEL_PRUNING_THREADS,
      POLYJAM_PARALLEL_PRUNING_MIN_BLOCK_ROWS );

  std::list<int> usedEquations = polyjam::generator::pruning::toList(basisRows);
  std::cout << "Parallel row-basis pruning candidate height: "
            << equationCount << " -> " << usedEquations.size()
            << " using up to " << polyjam::generator::pruning::availableThreads()
            << " worker thread(s)." << std::endl;

  if( polyjamVerifyPrunedRows(big_matrix, usedEquations, goodPolynomials) )
  {
    std::cout << "Parallel row-basis pruning verified successfully." << std::endl;
    return usedEquations;
  }

  std::cout << "Parallel row-basis pruning verification failed; falling back to the original serial safe-removal pass." << std::endl;
  return polyjamSequentialEquationRemoval(big_matrix, goodPolynomials, equationCount);
#else
  return polyjamSequentialEquationRemoval(big_matrix, goodPolynomials, equationCount);
#endif
}
    ...
    bool visualize )
{
  ...
  //plus a list of the origin of the equations.  Prune the equation list by preserving the exact row space of
  //the Macaulay template; this is parallelized by block row-basis extraction when oneTBB is available.
  std::list<int> usedEquations = polyjamPruneEquations(big_matrix, goodPolynomials, equations.size());
  ...
  Actiontype << "Eigen::MatrixXd";
  code << Actiontype.str() << " Action = " << Actiontype.str() << "::Zero(" << solNbr << "," << solNbr << ");" << std::endl;
  ...
  file << std::endl;
  file << "  Eigen::EigenSolver<Eigen::MatrixXd> Eig(Action,true);" << std::endl;
  file << "  Eigen::VectorXcd D = Eig.eigenvalues();" << std::endl;
  file << "  Eigen::MatrixXcd V = Eig.eigenvectors();" << std::endl;
  file << std::endl;
  ...
  file << "      bool valid_solution = true;" << std::endl;
  file << std::endl;
  file << "      std::complex<double> temp;" << std::endl;

  int oneIndex = solNbr - 1;
  core::Monomial oneMonomial(unknownNbr);
  for( int b = 0; b < baseMonomials.size(); b++ )
  {
    if( baseMonomials[b] == oneMonomial )
    {
      oneIndex = b;
      break;
    }
  ...
  {
    // Search for the index of a first-order unknown in the quotient basis.
    // If it is not in the basis (for example, a unique AX=XB solution often
    // has quotient basis {1}), recover it from the generated Groebner
    // reduction row: x_j = -sum_k M3(row,tail_k) * basis_k.
    std::vector<unsigned int> exponents( unknownNbr, 0 );
    exponents[d] = 1;
    core::Monomial unknown( exponents );

    int b = -1;
    for( int baseIndex = 0; baseIndex < baseMonomials.size(); baseIndex++ )
    {
      if( unknown == baseMonomials[baseIndex] )
      {
        b = baseIndex;
        break;
      }
    ...
    if( b >= 0 )
    {
      file << "      temp = V(" << b << ",c) / V(" << oneIndex << ",c);" << std::endl;
    }
    else
    {
      int leadingIndex = -1;
      for( int monoIndex = 0; monoIndex < finalMonomials.size(); monoIndex++ )
      {
        if( unknown == finalMonomials[monoIndex] )
        {
          leadingIndex = monoIndex;
          break;
        }
      }

      if( leadingIndex >= 0 && leadingIndex < M2rows )
      {
        file << "      temp = std::complex<double>(0.0,0.0);" << std::endl;
        for( int k = 0; k < solNbr; k++ )
        {
          file << "      temp -= M3(" << leadingIndex << "," << (M3cols - solNbr + k) << ") * V(" << k << ",c) / V(" << oneIndex << ",c);" << std::endl;
        }
      }
      else
      {
        file << "      valid_solution = false;" << std::endl;
        file << "      temp = std::complex<double>(0.0,0.0);" << std::endl;
      }
    }
  ...
  file << "      if(valid_solution) solutions.push_back(sol);" << std::endl;
  file << "    }" << std::endl;
  file << "  }" << std::endl;
  file << "}";
  file.close();
