...
polyjam::execGeneratorInternal( bool even, list<Poly*> & eqs, list<Poly*> & eqs_sym, int expanderDegree, std::vector<Monomial> & baseMonomials, const string & solverName, const string & suffix, const string & parameters, bool visualize )
{
  ...
  //create the action variable. Most Polyjam examples use the last unknown.
  //For the quaternion hand-eye solver, use qz (unknown x_4) instead: q and -q
  //are both valid rotations, while the translation variables are identical for
  //that sign pair and therefore make a poor action variable.
  vector<unsigned int> action(nu,0);
  if(solverName.find(string("handeye_axxb")) == 0 && !even && nu >= 4)
    action[3] = 1;
  else if(even)
    action.back() = 2;
  else
